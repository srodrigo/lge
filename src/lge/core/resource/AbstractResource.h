...
#ifndef _LGE_ABSTRACT_RESOURCE_H_
#define _LGE_ABSTRACT_RESOURCE_H_
...
public:
	 ...
	 */
	explicit AbstractResource(const std::string& filename)
	 ...
	 */
	AbstractResource(const std::string& filename, bool autoreleaseResource)
	...
	virtual AbstractResource* clone() const = 0;
	...
	{
		if (resource) {
			lge::log::debug("AbstractResource::release", "Removing resource pointer %p", resource);
			delete resource;
			resource = NULL;
			lge::log::debug("AbstractResource::release", "Resource pointer %p removed", resource);
		}
	}
